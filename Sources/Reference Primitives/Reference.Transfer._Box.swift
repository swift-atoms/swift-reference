...
/// State constants for Reference.Transfer._Box state machine.
///
/// Hoisted to module scope due to Swift limitation: static stored properties
/// are not supported in generic types. Refer via `Reference.Transfer._Box.State`.
@usableFromInline
enum __TransferBoxState {
...
extension Reference.Transfer {
    ...
    internal final class _Box<T: ~Copyable>: @unchecked Sendable {
        ...
        @usableFromInline
        typealias State = __TransferBoxState
        ...
        func store(_ value: consuming T) {
            ...
            if !reserved {
                if original == State.full || original == State.initializing {
                    preconditionFailure("Reference.Transfer: store() called when value already present")
                } else {
                    preconditionFailure("Reference.Transfer: store() called after take()")
                }
            }
        ...
        func take() -> T {
            ...
            if !exchanged {
                if original == State.empty || original == State.initializing {
                    preconditionFailure("Reference.Transfer: take() called when no value present")
                } else {
                    preconditionFailure("Reference.Transfer: take() called twice")
                }
            }
